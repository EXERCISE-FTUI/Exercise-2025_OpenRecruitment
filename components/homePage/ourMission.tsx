...
const BracketCard: React.FC<BracketCardProps> = ({number, message, delay}) => {
	...
	return (
		<motion.div
			...
			className={`${bracketColor} w-full h-auto pl-20 pr-6 py-3 rounded-3xl shadow-xl z-0 relative`}
		>
			<p className={`${textColor} text-md font-light`}>{message}</p>
...
const OurMission: React.FC = () => {
	...
	return (
		<motion.div
			...
			viewport={{once: true, margin: "-150px"}}
			className="w-full max-w-4xl mx-auto pb-32 flex flex-col justify-center items-center max-md:p-4 relative"
		>
